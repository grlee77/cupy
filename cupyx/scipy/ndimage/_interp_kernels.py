...
def _get_coord_zoom_and_shift(ndim, nprepad=0, float_type=None,
                              batch_axes=None, loop_batch_axis=False):
...
def _generate_interp_custom(
    ...
    # later)
    ops = ops + coord_func(
        ndim, nprepad, float_type=float_type, batch_axes=batch_axes,
    ...
    if order == 0:
        ...
        for j in spatial_axes:
            ...
            if mode != 'constant':
                ...
                if mode == 'wrap':
                    ...
                    {int_t} cc_bounded_{j} =
                        ({int_t})floor(dcoordf_{j} + ({float_type})1.0);;
                    '''
                    )
        ...
        for j in spatial_axes:
            ...
            if order & 1:
                ...
                op_str = '''
                start = ({int_t})floor({coord_var} + ({float_type})0.5)
                    - {order_2};'''
            ops.append(
                op_str.format(
                    int_t=int_t, coord_var=coord_var, float_type=float_type,
                    order_2=order // 2
                )
            )
...
def _get_affine_kernel(ndim, large_int, yshape, mode, cval=0.0, order=1,
                       integer_output=False, nprepad=0,
                       float_dtype=cupy.double, batch_axes=None):
