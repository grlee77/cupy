...
def _get_coord_map(ndim, nprepad=0, batch_axes=None, loop_batch_axis=False):
    ...
    W c_{j} = (W)in_coord[{j}]{pre};'''
            ...
            if loop_batch_axis:
                # Read coords at out_base_idx (first batch element at this
                # spatial pos)/ The spatial coords are the same for all batch
                # elements
                ops.append(
                    f'''
...
def _get_coord_zoom(ndim, nprepad=0, batch_axes=None, loop_batch_axis=False):
...
def _get_coord_shift(ndim, nprepad=0, batch_axes=None, loop_batch_axis=False):
...
def _get_coord_affine(ndim, nprepad=0, batch_axes=None, loop_batch_axis=False):
...
def _generate_interp_custom(
    ...
    # For loop_batch_axis, this only computes spatial coords (batch coord set
    # later)
    ops = ops + coord_func(
        ndim, nprepad, batch_axes=batch_axes, loop_batch_axis=loop_batch_axis
    )
    ...
    if order == 0:
        ...
        if loop_batch_axis:
            # Optimized path:
            # Spatial interpolation loops outside, batch loop innermost.
            ...
            if integer_output:
                ops.append(
                    f'''
            for ({uint_t} batch_idx = 0; batch_idx < batch_size; batch_idx++) {{
                y[out_base_idx + batch_idx] = (Y)rint((double)out_batch[batch_idx]);
            }}'''  # noqa: E501
                )
            else:
                ops.append(
            ...
            {{  // dummy scope for batch axis {j}'''
                    ...
                    # spatial axes are already set up, just add the loop
                    ops.append(
                        f'''
            for (int s_{j} = 0; s_{j} < n_{j}; s_{j}++)
                {{
        ...
        # Compute spline weights and indices for spatial axes
        # (outside batch loop)
        for j in spatial_axes:
        ...
        if loop_batch_axis:
            ...
            # For order > 1, we need bounds check for both constant and
            # grid-constant modes
            ...
            {{  // dummy scope for batch axis {j}'''
                    ...
                    # spatial axes already have weights/indices computed, just
                    # add the loop
                    ops.append(
                        f'''
            W w_{j};
    ...
    # Output writing (for non-looped cases - looped cases write inside their
    # batch loops)
    if not loop_batch_axis:
        if integer_output:
...
def _get_map_kernel(ndim, large_int, yshape, mode, cval=0.0, order=1,
                    integer_output=False, nprepad=0, batch_axes=None):
    ...
    kernel = cupy.ElementwiseKernel(in_params, out_params, operation, name,
